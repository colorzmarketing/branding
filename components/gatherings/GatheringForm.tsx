...
export default function GatheringForm({ initial, onSubmit, onCancel }: Props) {
  ...
  const [form, setForm] = useState<GatheringFormData>({
    ...
    cost: initial?.cost ?? 0,
    pipeline_completed: initial?.pipeline_completed ?? null,
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              set("target_participants", e.target.value ? Number(e.target.value) : null)
            }
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              set("target_profit", e.target.value ? Number(e.target.value) : null)
            }
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
          />
        </div>
